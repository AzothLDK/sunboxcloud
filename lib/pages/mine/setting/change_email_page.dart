...
class _ChangeEmailPageState extends State<ChangeEmailPage> {
  ...
  void initState() {
    ...
    _currentEmail = authController.userInfo.value?['email'] ?? '';
  }

  // Future<void> _loadUserInfo() async {
  //   try {
  //     final response = await ApiService.getSunboxLoginInfo();
  ...
  Future<void> _sendVerificationCode() async {
    ...
    if (email == _currentEmail) {
      ToastUtils.error('new_email_same_as_old'.tr);
      return;
    }
  ...
  Future<void> _editEmail() async {
    try {
      ...
      if (response['code'] == 200) {
        ...
        ToastUtils.error(response['msg'] ?? 'email_change_failed'.tr);
      }
    } catch (e) {
      ToastUtils.error('email_change_failed'.tr);
    }
  }
